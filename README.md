...
如果將同一閉環的所有車格抓出來(也就是第一個車格去年的住戶會是最後一個車格今年的住戶)，則此閉環愈大(串了愈多車格)，更換車位時就有可能會卡得愈嚴重。不幸的是，經過數值模擬發現，如果抽籤結果愈接近隨機，就愈有可能發生閉環很長的情況。經過分析，2025年更換車位時，最長的閉環足足有105個車格(佔所有車格的82%)，如果這105戶沒有同時移車，是沒辦法順利完成更換的。
...
演算法的核心概念是限制最長的閉環數量，讓更換車格時不會因為其中一格的機車遲遲未移動，而影響後面所有的機車，簡要流程如下：
1. 羅列所有參與抽籤的車格和所對應去年使用的住戶
2. 亂數排序車格，成為今年的序列
3. 將此序列隨機取2-5的數量分組，每一組強制構成一個閉環
4. 取出同一組的車格，以形成閉環的方式填入該車格今年的住戶(下一順位車格的舊住戶是前一順位車格的新住戶；最後順位車格的新住戶是第一順位車格的舊住戶)
5. 補回遞補與預抽籤的車格、彼此互換的車格、未更動的車格 <br>

完成！(細節請見程式碼main.py)

## Limitations
演算法仍難以避免有如下限制：
1. 抽籤的過程僅僅是開啟一個終端機視窗，下一道指令，執行完就結束了，對於不熟悉程式的人會非常空虛，無法enjoy抽籤的過程
2. 亂數結果會受初始條件(去年的抽籤結果和保留車位)的影響，無法達到每個車格的隨機性完全相同，但不會差很多
3. 目前只有演算法，尚無前端UI，使用門檻較高，待後續管委會或住戶開發更友善的介面，以利往後每年抽籤
4. 演算法會強制更換車格，無法保留隨機抽籤仍有極小的機率可以抽中同一車格的可能性
...
4. 程式會自動建立「抽籤結果」資料夾，並輸出抽籤結果，包含四個檔案(假設為2026年抽籤)：
   ...
   > 歷年機車位(戶號).xlsx <br>
   > 歷年機車位(門牌).xlsx <br>
5. 住戶可於抽籤後互換車格(僅供兩兩互換)，請編輯「初始化設定」資料夾內的「互換車格.txt」檔案，將欲互換的車格號碼填入，以(半形)逗號分隔，並執行程式
   ```
   python exchange.py
   ```
   > 若非同一閉環內的車格互換，或換回與去年相同的車格，互換後會合併兩個車格所屬的閉環，形成一個長度相加，較大的閉環。因此互換後無法確保閉環的車格數量仍為5個以下
...
## 溫馨提醒
本程式已盡可能考慮所有可能發生的情況避免出錯，也模擬過多次抽籤(large_random_test.py)並於每次抽籤完做sanity check並無發現異狀，但仍難以確保任何初始條件下皆可正常運作，若住戶對於抽籤結果有疑慮，或發現異常狀況，請第一時間向管委會反應。